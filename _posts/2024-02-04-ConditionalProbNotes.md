...
However, a few points of confusion came up as I learned about generative models and conditional probabilities (see [this post](https://freeenergy.blog/2026/EMnotes.html){:target='_blank'}).
...
So let's assume that we start with the joint probability density $$P(x,z)$$, which is determined by how the images fit into the various classifications. Then, since we decided to generate a rose image, which means that $$z$$ is definitely the rose value $$z_0$$, we need to set $$P(z)$$ to be a delta function at $$z_0$$. But $$P(z)$$ is supposed to be the marginal distribution, calculated by integrating $$P(x,z)$$ over $$x$$, so it cannot be affected by our arbitrary choice of $$z_0$$. So there is a contradiction...
